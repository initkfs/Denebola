 ...
 * Authors: initkfs
 */
module api.os.math.math_float;

// dfmt off
version (VerFPU):
// dfmt on

import api.os.errors;

import MathCore = api.os.math.math_core;
import MathFloatExterns = api.os.math.math_float_externs;
...
auto ln(T)(T x) if (__traits(isArithmetic, T))
{
    ...
    {
        import Logd = api.os.math.libm.logd;

        return Logd.__ieee754_log(x);
    }
    else static if (is(T : float))
    {
        import Logf = api.os.math.libm.logf;
